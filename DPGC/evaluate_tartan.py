...
import time  # <--- 1. 确保在文件顶部导入 time 模块
...
#         # --- FPS 计时开始 ---
#         torch.cuda.synchronize()  # 等待之前的 GPU 操作完成，确保计时准确
#         start_time = time.time()

#         # 原有的 SLAM 处理逻辑
#         # with Timer("SLAM", enabled=False): # 可以把这行注释掉或者忽略
#         slam(t, image, intrinsics)

#         torch.cuda.synchronize()  # 等待当前帧的 GPU 计算完成
#         end_time = time.time()
        
#         # 计算当前帧耗时并保存 (单位: 秒)
#         cost_time = end_time - start_time
#         frame_times.append(cost_time)
#         # --- FPS 计时结束 ---

#     # --- 计算统计数据 ---
#     if len(frame_times) > 0:
#         #通常第一帧因为预热（warmup）会很慢，计算平均 FPS 时建议去掉前几帧（例如前5帧）
#         valid_times = frame_times[5:] if len(frame_times) > 5 else frame_times
#         avg_time = np.mean(valid_times)
#         fps = 1.0 / avg_time
#         print(f"[{imagedir}] Processed {len(frame_times)} frames. Average Inference FPS: {fps:.2f} (Avg Latency: {avg_time*1000:.2f} ms)")
...
#CUDA_VISIBLE_DEVICES=7 python evaluate_tartan.py     --weights /home/ghb/DPVO/checkpoints/pALL_135000.pth    --trials 1         --split test  --save_trajectory  --plot
