...
def train(args):
    ...
    train_loader = DataLoader(db, batch_size=1, shuffle=True, num_workers=4) ##原来为4
    ...
    for epoch in range(args.steps):  # You might want to track epochs too, not just steps
        ...
        with tqdm(total=len(train_loader), desc=f'Epoch {epoch+1}/{args.steps}') as pbar:
            for data_blob in train_loader:
                ...
                # if total_steps % 10000 == 0: ####10000
                # import pdb; pdb.s
                # duan = 0
                if ((total_steps) <= 200000 and (total_steps) % 10000 == 0) or ((total_steps) > 200000 and (total_steps) % 5000 == 0):
                    torch.cuda.empty_cache()

                    if rank == 0:
                        PATH = 'checkpoints/%s_%06d.pth' % (args.name, int(total_steps))
