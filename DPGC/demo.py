...
@torch.no_grad() #禁用梯度计算，节省显存并提高性能。在推理（而非训练）模式下尤其适用。
def run(cfg, network, imagedir, calib, stride=1, skip=0, viz=False, timeit=False):

    slam = None
    ## 创建队列 Queue 是一个多进程安全的队列，可以用于在进程间共享数据。
    ...
    while 1:
        ...
        # print('hhhh',image.shape) 
        # exit() torch.Size([3, 640, 352])
...
# python demo.py --imagedir=/home/ghb/DPVO/ipone.mp4 --calib=calib/iphone.txt --stride=1 --plot --save_trajectory

# python demo.py --imagedir=test2.mp4  --network /home/ghb/DPVO/checkpoints/pALL_150000.pth --calib=calib/iphone.txt --stride=1 --plot --save_trajectory 