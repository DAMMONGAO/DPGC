...
def run(cfg, network, imagedir, calib, stride=1, viz=False, show_img=False):
    ...
    while 1:
        ...
        if image is None or intrinsics is None:
                print("Warning: Received None for image or intrinsics, skipping this frame.")
                continue
...
if __name__ == '__main__':
    ...
    for scene in scenes:
        ...
        for i in range(args.trials):
            ...
            # print("Checking for NaN or Inf in traj_est:", np.any(np.isnan(traj_est.positions_xyz)) or np.any(np.isinf(traj_est.positions_xyz)))
            # print("Checking for NaN or Inf in traj_ref:", np.any(np.isnan(traj_ref.positions_xyz)) or np.any(np.isinf(traj_ref.positions_xyz)))
            # print("Positions XYZ range in traj_est:", np.min(traj_est.positions_xyz), np.max(traj_est.positions_xyz))
            # print("Positions XYZ range in traj_ref:", np.min(traj_ref.positions_xyz), np.max(traj_ref.positions_xyz))
    ...
    print(f"Max Allocated: {max_allocated:.2f} MB, Max Cached: {max_cached:.2f} MB")


#CUDA_VISIBLE_DEVICES=7  python evaluate_icl_nuim.py     --network /home/ghb/DPVO/checkpoints/pALL_135000.pth --trials 1  --save_trajectory --plot